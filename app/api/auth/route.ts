...
import { accountLogin, revokeAccountSession } from "@/lib/account-access";
...
export async function POST(request: Request) {
  ...
  if (body && typeof body.user === "string" && typeof body.password === "string") {
    try { token = credentialsValid(body.user, body.password) ? createSession() : await accountLogin(body.user, body.password); }
    catch { return NextResponse.json({ error: "No se pudo verificar la cuenta. Inténtalo de nuevo." }, { status: 503 }); }
  }
  if (!token) {
...
export async function DELETE(request: Request) {
  ...
  try { await revokeAccountSession(request); } catch { return NextResponse.json({ error: "No se pudo cerrar la sesión. Inténtalo de nuevo." }, { status: 503 }); }
