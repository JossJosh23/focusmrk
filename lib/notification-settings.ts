...
export type NotificationSettings = { start: string; end: string; dailyLimit?: number; rules: Record<NotificationKind, { intensity: Intensity; time: string }> };
export const defaultNotificationSettings: NotificationSettings = {
  start: "08:00", end: "20:00", dailyLimit: 4,
...
export function validNotificationSettings(value: unknown): value is NotificationSettings {
  ...
  return (s.dailyLimit === undefined || (Number.isInteger(s.dailyLimit) && s.dailyLimit >= 1 && s.dailyLimit <= 12)) && time(s.start) && time(s.end) && s.start < s.end && !!s.rules && notificationKinds.every(k => {
