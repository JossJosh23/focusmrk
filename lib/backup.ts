...
export function parseBackup(raw: string): Backup {
  ...
  for (const a of value.assets) {
    if (!a || typeof a.id !== "string" || !a.id || typeof a.name !== "string" || typeof a.brand !== "string" || !a.brand.trim() || a.brand.length > 80 ||
      !MEDIA_TYPES.includes(a.type) || typeof a.createdAt !== "string" || !Number.isInteger(a.size) || a.size <= 0 || a.size > MAX_MEDIA_BYTES ||
  ...
  return { format: "focusmrk-backup", version: 1, createdAt: value.createdAt, posts, templates, assets: value.assets };
}
