...
export function CompanySelector({ server, known, value, onChange, canCreate = true }: { server: boolean; known: string[]; value: string; onChange: (value: string) => void; canCreate?: boolean }) {
  ...
  return <div className="company-selector company-switcher">
    <details ref={menu} className="company-menu" onToggle={event => { if (event.currentTarget.open) { setQuery(""); searchInput.current?.focus(); } }} onKeyDown={event => { if (event.key === "Escape" && menu.current?.open) { event.preventDefault(); event.stopPropagation(); closeMenu(); } }}>
      <summary aria-label={`Cambiar empresa: ${value || "Todas las empresas"}`}><span className="company-avatar">{value ? value.slice(0, 2).toLocaleUpperCase("es") : <Layers size={19} />}</span><span className="company-trigger-text"><small>ESPACIO DE TRABAJO</small><strong title={value || "Todas las empresas"}>{displayName || value || "Todas las empresas"}</strong></span><ChevronDown size={15} className="company-chevron" /></summary>
      <div className="company-dropdown"><div className="company-dropdown-heading"><strong>Tus empresas</strong><span>{options.length}</span></div><label className="company-search"><Search size={15} /><input ref={searchInput} type="search" aria-label="Buscar empresa" placeholder="Buscar empresa..." value={query} onChange={event => setQuery(event.target.value)} aria-controls={listId} /></label>
    ...
    }}><input autoFocus aria-label="Nombre de la empresa" placeholder="Nombre de la empresa" required maxLength={80} value={name} disabled={busy} onChange={event => setName(event.target.value)} /><div><button type="submit" disabled={busy || !name.trim()}>Guardar</button><button type="button" disabled={busy} aria-label="Cancelar" onClick={() => setAdding(false)}><X size={14} /></button></div></form>}{error && <p role="alert">{error}</p>}
  </div>;
}
