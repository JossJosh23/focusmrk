...
export function ReminderPanel({ posts, onOpen, onConfigure }: { posts: Publication[]; onOpen: (post: Publication) => void; onConfigure: () => void }) {
  ...
  return <div className="reminder-bell-container">
    ...
    <div ref={panel} id={panelId} popover="auto" className="reminder-panel reminder-center reminder-popover" role="region" aria-labelledby={panelId + "-title"}>
      <div className="reminder-popover-heading"><div><strong id={panelId + "-title"}>Recordatorios</strong><p>{now === null ? "Cargando?" : total ? total + " publicaciones pendientes" : "Sin pendientes ahora"}</p></div><button type="button" className="icon-button" popoverTarget={panelId} popoverTargetAction="hide" aria-label="Cerrar recordatorios"><X size={18} /></button></div>
