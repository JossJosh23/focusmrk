...
export function CalendarViews({ view, month, today, posts, ready, onEdit, onMove }: {
  ...
  const postsByDate = useMemo(() => {
    ...
    for (const post of posts) {
      const daily = grouped.get(post.date) || [];
      daily.push(post);
      grouped.set(post.date, daily);
    }
    return grouped;
  }, [posts]);
  ...
  return <div id="calendar-results" className={`calendar-scroll ${view === "week" ? "week-view" : ""}`} tabIndex={0} role="region" aria-label={view === "week" ? "Calendario semanal" : "Calendario mensual"}>
    <div className="calendar-grid">
      ...
      <div className="days-grid">{month && (view === "week" ? weekDays(month) : monthDays(month)).map((date) => {
        ...
        const daily = postsByDate.get(key) || [];
