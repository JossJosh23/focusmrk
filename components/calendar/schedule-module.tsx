...
export function ScheduleModule({ posts, today, onCreate, onEdit }: { posts: Publication[]; today: string; onCreate: () => void; onEdit: (post: Publication) => void }) {
  ...
  return <section className="schedule-module schedule-studio">
    <div className="page-heading"><div><span className="eyebrow">PLANIFICA Y PRESENTA</span><h1>Cronogramas<span>.</span></h1><p>Elige el periodo, revisa tus publicaciones y prepara tu entrega.</p></div><button type="button" className="secondary-button" disabled={!!busy} onClick={onCreate}><Plus size="var(--icon-md)" />Crear publicación</button></div>
