...
export function PushSettings({ onActive }: { onActive: (value: boolean) => void }) {
  ...
  return <section className="push-settings"><strong>Avisos al celular</strong><p>Recibe los avisos configurados en este módulo, aunque cierres FocusMRK.</p><div className="template-actions"><button type="button" className="secondary-button" disabled={!reg || busy} onClick={() => void toggle()}>{busy ? "Procesando…" : active ? "Desactivar en este dispositivo" : "Activar en este dispositivo"}</button>{active && <button type="button" className="secondary-button" disabled={busy} onClick={() => void test()}>Enviar prueba</button>}</div>{message && <p role="status">{message}</p>}</section>;
}
