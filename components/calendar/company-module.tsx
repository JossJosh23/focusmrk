...
export function CompanyModule({ company, known, server, onChange }: { company: string; known: string[]; server: boolean; onChange: (company: string) => void }) {
  return <section className="company-module"><div className="page-heading"><div><span className="eyebrow">TU CLIENTE, EN UN SOLO LUGAR</span><h1>Empresa<span>.</span></h1><p>Gestiona su identidad, sitio web y presencia en redes.</p></div></div>
    ...
    {company ? <CompanyForm key={company} company={company} server={server} /> : <div className="company-empty"><Building2 size={40} /><h2>Elige la empresa que vas a gestionar</h2><p>Usa el selector para elegir una empresa o añadir tu primer cliente.</p></div>}
  </section>;
}
...
function CompanyForm({ company, server }: { company: string; server: boolean }) {
  ...
  return <form className="company-profile-form" onSubmit={async event => {
    ...
    <fieldset disabled={!ready || busy} className="company-identity"><h2><Building2 size={19} />Identidad de la empresa</h2><div className="company-logo-row"><div className="company-logo">{profile.logo ? <Image unoptimized src={profile.logo} alt="Logo de la empresa" width={96} height={96} /> : <Building2 size={34} />}</div><div><label className="company-logo-upload"><Upload size={15} />Subir logo<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async event => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; if (!["image/png", "image/jpeg", "image/webp"].includes(file.type) || file.size > 500000) { setMessage("Usa un logo PNG, JPG o WebP de hasta 500 KB."); return; } try { const data = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(file); }); field("logo", data); } catch { setMessage("No se pudo leer el logo."); } }} /></label><small>PNG, JPG o WebP · Hasta 500 KB</small>{profile.logo && <button type="button" className="company-remove-logo" onClick={() => field("logo", "")}>Quitar logo</button>}</div></div>
    <label>Nombre de la empresa<input required maxLength={80} value={profile.name} onChange={event => field("name", event.target.value)} /></label><label>Descripción<textarea rows={3} maxLength={1000} value={profile.description} onChange={event => field("description", event.target.value)} placeholder="Qué hace la empresa y cómo se comunica" /></label><label><span><Globe size={15} />Página web</span><input type="url" placeholder="https://tuempresa.com" value={profile.website} onChange={event => field("website", event.target.value)} /></label>{profile.website.startsWith("https://") && <a href={profile.website} target="_blank" rel="noopener noreferrer">Visitar sitio web <ExternalLink size={13} /></a>}</fieldset>
    <fieldset disabled={!ready || busy} className="company-socials"><h2>Redes sociales</h2><p>Guarda los perfiles oficiales de esta empresa.</p>{([['instagram', 'Instagram'], ['facebook', 'Facebook'], ['tiktok', 'TikTok']] as const).map(([key, label]) => <div className="company-social-card" key={key}><div><span className={`company-social-icon ${key}`}><SocialPlatformIcon platform={label} size={23} /></span><strong>{label}</strong><span className="company-link-status">{profile[key] ? "Enlace añadido" : "Sin enlace"}</span></div><label>Enlace del perfil<input type="url" value={profile[key]} placeholder={`https://www.${key}.com/...`} onChange={event => field(key, event.target.value)} /></label>{profile[key] && validCompanyProfile({ ...emptyCompanyProfile(company), [key]: profile[key] }) && <a href={profile[key]} target="_blank" rel="noopener noreferrer">Abrir perfil <ExternalLink size={13} /></a>}<small>Sin conexión autorizada · Métricas y publicación no disponibles</small></div>)}<div className="company-connection-note"><strong>Conexión de cuentas</strong><p>Guardar un perfil no autoriza el acceso a la cuenta. La conexión para obtener métricas y publicar se habilitará después de configurar las integraciones.</p></div></fieldset>
    <div className="company-save-bar"><span role="status">{message || (!ready ? "Cargando empresa…" : dirty ? "Cambios sin guardar" : "Ficha actualizada")}</span><button type="submit" className="primary-button" disabled={!ready || busy || !dirty}><Save size={16} />{busy ? "Guardando…" : "Guardar cambios"}</button></div>
  </form>;
}
