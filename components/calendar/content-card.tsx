...
export function ContentCard({ post, onEdit, agenda = false }: {
  post: Publication; onEdit: (post: Publication) => void; agenda?: boolean;
}) {
  return <article className={agenda ? "agenda-post" : "content-item"}>
    <button type="button" className={`post-card ${agenda ? "agenda-card" : ""}`} onClick={() => onEdit(post)} aria-label={`Editar ${post.title}`}>
